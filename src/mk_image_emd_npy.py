...
def images_preprocessing(root_path):
    photo_dir = 'images'
    sketch_dir = 'sketches'
    photo_sd = ''
    sketch_sd = ''
    ...
    # 이미지들을 tensor화 하는 단계
    data_test_image = DataGeneratorImage(dataset, root_path, photo_dir, photo_sd, all_files_image,
...
dict_clss = create_dict_texts(splits['tr_clss_im'])
...
# for param_tensor in sem_pcyc_model.state_dict():
#     print(param_tensor, "\t", sem_pcyc_model.state_dict()[param_tensor].size())
# print(torch.load(path_pth))
...
sem_pcyc_model.eval()
acc_im_em = images_preprocessing(root_path)
print("--------------END Embedding--------------")
print("\n")
print("--------------START Saving--------------")
image_em = np.save("/home/ubuntu/projects/image_embedding.npy", acc_im_em)
