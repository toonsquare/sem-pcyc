...
def main():
    ...
    test_input_image = ''
    if args.test_input:
        test_input_path = os.path.join(os.getcwd(), args.test_input)
        test_input_image = Image.open(test_input_path)
        test_input_image = ImageOps.invert(test_input_image).convert(mode='RGB')
    ...
    print('Convert test input house.png')
    test_input_image = transform_sketch(test_input_image)
...
def validate(valid_loader_sketch, valid_loader_image, sem_pcyc_model, epoch, args, test_input=False):
    ...
    print('test_input_em success shape : {}'.format(test_input.shape))
    ...
    # Compute similarity
    t = time.time()
    sim_euc = np.exp(-cdist(test_input_em, acc_im_em, metric='euclidean'))
