...
def main():
    ...
    parser = argparse.ArgumentParser(description='Sketch Model.')
    ...
    parser.add_argument('--model', default='SEResNet', help='Type of model')
    ...
    path_cp = os.path.join(path_aux, 'CheckPoints', args.dataset, 'sketch', args.model)
    path_log = os.path.join(path_aux, 'LogFiles', args.dataset, 'sketch', args.model)

    # Parameters for transforming the images
    sk_mean = [0.485, 0.456, 0.406]  # RGB channel mean for Imagenet
    sk_std = [0.229, 0.224, 0.225]  # RGB channel std for Imagenet
    transform = transforms.Compose([transforms.Resize((args.sk_sz, args.sk_sz)), transforms.ToTensor(), transforms.Normalize(sk_mean, sk_std)])
    ...
    if args.dataset == 'Sketchy':
        ...
        splits = utils.load_files_sketchy(root_path=root_path, sketch_dir=sketch_dir, sketch_sd=sketch_sd)
    ...
    if args.model == 'ResNet50':
        model = ResNet50Feats(num_classes=len(dict_clss), pretrained=True, finetune_feats=True, finetune_class=True)
    ...
    if os.path.isfile(best_model_file):
        ...
        model.load_state_dict(checkpoint['state_dict_sketch'])
